#!/usr/bin/env bash
# ============================================================================
#  Jihad al-Dawah Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/28-jihad-al-dawah}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.jihadaldawah}"
AVD_NAME="${AVD_NAME:-jihad_al_dawah_test}"
...
cmd_test() {
  ...
  local out="/tmp/jihad-al-dawah-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Jihad al-Dawah Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
